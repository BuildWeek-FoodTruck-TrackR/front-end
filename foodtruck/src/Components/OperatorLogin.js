...
const OperatorLogin = (props) => {
  ...
  const formSubmit = e => {
    e.preventDefault();
    props.postLogin(formData, FetchOperatorLoginType, 'auth/operator/login');
  }
  ...
  return <>
    <Form onSubmit={formSubmit} className="mt-5">
      ...
      <Button className="mt-4" color={buttonDisabled ? "secondary" : "primary"}>Login</Button>
    </Form>
  </>
}

export default connect(mapStateToProps, { postLogin })(OperatorLogin);